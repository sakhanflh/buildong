...
export function Notifications({ showNotifications, showNotif }) {
    const [visibleComponent, setVisibleComponent] = useState('GeneralNotif')

    return (
        <>
            <div className={`fixed xl:absolute  font-jost bg-white shadow-2xl xl:rounded-xl xl:translate-x-[35rem] translate-y-16 p-5 transition-all duration-500  ${showNotifications ? 'w-dvw h-dvh xl:w-[30rem] xl:h-[32rem] z-50 xl:opacity-100' : 'w-52 h-0 xl:translate-x-[50rem] xl:h-0 -z-50 overflow-hidden opacity-0 cursor-none'}`}>
                <div>
